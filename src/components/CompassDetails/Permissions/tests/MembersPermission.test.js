...
import MembersPermission from "../NewMemberPermission";
...
describe('testing Compass Section', () => {
  ...
  test('Show default Compass Permission', async () => {
    ...
    const { getByText } = renderUnit(<MembersPermission />, props)

    const CompassSteps = await waitForElement(() => getByText("Add Member"))
