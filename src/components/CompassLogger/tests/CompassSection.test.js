...
describe('testing Compass Section', () => {
  ...
  test('Show default values with given interactions/compass/session', async () => {
    ...
    const RecentLogs = await waitForElement(() => getByText("Recent Logs"));
    ...
    expect(RecentLogs).toBeInTheDocument();
    ...
    expect(Prototype.length).toBe(3);
    expect(GraspChallenge.length).toBe(3);
