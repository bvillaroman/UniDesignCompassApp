...
describe('testing Session Bar', () => {
  ...
  test('Show default values no interaction', async () => {
    ...
    const SessionName = await waitForElement(() => getByText("untitled"));
    const Header = await waitForElement(() => getByText("Logger"));
    const EnterLog = await waitForElement(() => getByPlaceholderText("Enter Log"));
    const Attachments = await waitForElement(() => getByText("Attachments"));
  ...
  test('Show default values with an interaction', async () => {
    const compass = {
      "id": "ca43a27b-9a67-4e09-bbb1-d3390b048b89",
      "name_of_compass": "testing",
      "description_of_compass": "save test",
    ...
    compassProps.compass = compass
    compassProps.session = session
    compassProps.interaction = interactions[0] 
    compassProps.interactions = interactions 
    ...
    const SessionName = await waitForElement(() => getByText("untitled"));
    const Header = await waitForElement(() => getByText("Grasp Challenge"));
    const EnterLog = await waitForElement(() => getByText("lkj"));
    const Attachments = await waitForElement(() => getByText("Attachments"));
